...
The collection can be a string, a list, or a dictionary. If the collection is a string or array, the third 
parameter is a starting index for where to search from. If the collection is a dictionary, the function 
searches for the value among values in the dictionary; since objects have no sort order, the third parameter 
is ignored.
...
def includes(collection, value, index=0):
    if isinstance(collection, dict):
        if value in list(collection.values()):
            return True
    elif value in collection[index:]:
        return True
    return False
